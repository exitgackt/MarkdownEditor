...
flowchart TD
    A[ページ表示] --> B{メンテナンス中?}
    B -->|Yes| C[P-004へリダイレクト]
    B -->|No| D[ログイン画面表示]
    D --> E[Googleログインボタンクリック]
    E --> F[Google OAuth画面]
    F --> G{認証成功?}
    G -->|Yes| H{初回ユーザー?}
    G -->|No| I[エラーメッセージ表示]
...
```mermaid
flowchart TD
    A[フォルダを開くクリック] --> B[File System Access API]
    B --> C[ディレクトリピッカー表示]
    C --> D{フォルダ選択?}
    D -->|Yes| E[ファイルツリー構築]
    D -->|No| F[キャンセル]
...
```mermaid
flowchart TD
    A[ページ表示] --> B[メンテナンス情報取得]
    B --> C[メンテナンス画面表示]
    C --> D{再読み込みクリック?}
    D -->|Yes| E[メンテナンス状態確認]
    E --> F{メンテナンス終了?}
    F -->|Yes| G[P-001へリダイレクト]
    F -->|No| C
    D -->|No| H[待機]
    H --> I{自動リロード?}
    I -->|60秒経過| E
...
flowchart TD
    ...
    G[編集ボタンクリック] --> H[編集ダイアログ表示]
    H --> I[内容編集]
    I --> J{保存?}
    J -->|Yes| K[API呼び出し]
    K --> L{成功?}
    L -->|Yes| M[設定更新・ダイアログ閉じる]
    L -->|No| N[エラー表示]
    J -->|No| O[ダイアログ閉じる]
```
